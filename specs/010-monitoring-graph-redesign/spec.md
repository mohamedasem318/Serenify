...
- Q: At the very start of warming, the **first** poll yields a single warming point. Under fill-to-width that lone point pins to the right edge as a short dashed **stub**, then **snaps** to a full-width dashed line when the 2nd point arrives — visually jarring. How should exactly 1 warming point render? → A: **Don't render the stub.** At **exactly 1 warming point** (warming-only, no confident reading yet), show the existing **just-started text state** (FR-018's "Your trend builds as readings come in." — no axes, no line, no stub); the dashed warming line renders only at **≥2 warming points** (where it already appears full-width). One clean appearance, no stub-then-snap. The subtitle still follows FR-024 (warming → "getting a read"). **Rationale:** a single point can't form a line, and the lone stub-then-snap is jarring; collapsing the 1-point case to the existing text body removes it with no new surface. **Scope:** the 1-warming-point case only — FR-019 confident single-dot, the ≥2-point warming line, and leading-skip/no-read treatments are unchanged. (FR-010; FR-018; the leading-warming-run-of-1-before-a-confident-reading is a separate, out-of-scope case — see the implementation flag.)

- Q: During session ramp-up (fewer than N_target = 12 capture windows, ~the first 2 minutes at the ~10s window stride), the current geometry anchors windows to the right at fixed `plot_width / N_target` slots, leaving the left of the plot blank (a "cut off" look every session for ~2 min). How should the early windows lay out? → A: **Fill-to-width during ramp-up** — the few early windows **stretch to fill the full plot width** (earliest at the left edge, latest at the right edge), with the **"now" marker pinned at the right edge**; all windows stay uniform-width slots so a no-read gap stays proportional to its consecutive-no-read count (scaled to the current fill). Existing points gently re-space (compress) as each new window arrives (by-design). At **N_target** the slot pitch **locks** at `plot_width / (N_target − 1)` and older windows thereafter scroll off the left (today's steady-state, unchanged); the ramp-up pitch `plot_width / (count − 1)` equals the locked pitch at count = N_target, so the transition is **continuous** (no jump). **Rejected alternatives:** *fill-from-left* (line grows left→right, the now-dot travels for ~2 min) — rejected because it moves the "you are here" anchor; *keep right-anchored blank-left* (current) — rejected for the "cut off" look every session for ~2 min. **Rationale for the pick:** the now-dot is the "you are here" anchor — keep it dead-still; accept gentle background re-spacing as the cost. (FR-002a ramp-up clause; Edge Cases "Ramp-up (sub-window) fill"; SC-012/SC-012a.)
...
- **Single confident reading**: render a single dot, not a line. This also covers a **lone confident reading flanked by no-reads** (an isolated confident window between gaps): a single point cannot be a line, so it renders as a dot at its band — extending FR-019's single-reading rule to any isolated confident point.
- **Fully read-less session** (warming-only or all-skipped, no confident band ever): render honestly as a no-read state, never as calm / at-ease. *(Exception, FR-018: a read-less session of **exactly one warming point** shows the just-started **text** state — "Your trend builds as readings come in." — not a dashed line; that text is still honest and never calm, so the never-calm guarantee holds.)*
...
- **Now marker when the last confident reading scrolled off (FR-004a × FR-002a)**: during a no-read sustained long enough that the last confident reading has **scrolled off the rolling window**, there is no on-screen reading to anchor to, so the marker shows **nothing** (no parked marker pointing off-screen) — the honest "nothing visible to point at." The parked marker (FR-004a) appears only while the last confident reading is still within the drawn ~2-min window; once it scrolls off, the marker disappears (and reappears when a confident reading returns).
- **Container resize / responsive**: the graph re-renders at the new fixed-pixel width; markers stay true circles and the graph keeps matching the camera stage width. *(Narrow-width axis sizing, decided 2026-06-25):* the left **axis gutter and right margin shrink responsively** below a width threshold (toward minimums at the 360px floor) so the plot keeps enough width for **legible no-read gap labels** at narrow widths; the left axis labels are retained (the minimum gutter still fits "A little tense"). Wide widths keep the full mock gutters.
- **Ramp-up (sub-window) fill**: while a young session has **fewer than N_target (= 12) capture windows** (~the first 2 minutes at the ~10s window stride), the few windows **stretch to fill the full plot width** (earliest at the left edge, latest at the right edge) with the **"now" marker pinned at the right edge** — NOT fixed `plot_width / N_target` slots anchored right, which would leave the left of the plot blank ("cut off") for ~2 min every session. Existing points **gently re-space** as each new window arrives (by-design). At **N_target** the slot pitch **locks** (`plot_width / (N_target − 1)`) and thereafter the oldest windows scroll off the left (the steady-state "Long session" behaviour below); the pitch is **continuous at the lock point** — no jump (FR-002a; SC-012a).
...
- **FR-002a** *(clarified 2026-06-25; x-axis model decided 2026-06-25)*: Because the session grows unboundedly while the graph width is fixed (FR-002), the trend MUST render only a **rolling window of the most recent ~2 minutes' worth of capture windows** (**N_target = 12** windows at the ~10s capture-**window stride** — 12 × ~10s ≈ 2 min; note this is the *capture-window* cadence, **not** the ~12s client poll/re-fetch cadence) rather than compressing the whole session into the fixed width. The x-axis uses a **uniform slot per capture window**: every capture window — confident reading **and** no-read window alike — occupies one equal-width horizontal slot, so spacing is **stable** and a no-read gap's width is simply its **count of consecutive no-read windows**. Because capture windows are ~uniform in duration, that slot count is an **honest proxy for elapsed time** (a longer outage is a wider gap) without a continuous time axis. Windows older than the rolling window **scroll off the left edge**. If the window ever renders too dense to keep the no-read **gap labels legible**, the implementation MUST **drop the oldest windows (shorten the window) rather than shrink the slot width** — legibility (SC-003) wins over completeness. The live "now" marker always sits at the right edge (the latest window).

  **Ramp-up (sub-window) fill** *(decided 2026-06-27)*: While the visible window count is **below N_target (= 12 windows, ~the first 2 minutes at the ~10s window stride)**, the windows MUST **stretch to fill the full plot width** — the earliest visible window at the **left edge**, the latest at the **right edge** — rather than sitting in fixed `plot_width / N_target` slots anchored to the right (which leaves the left of the plot blank, the "cut off" look that occurs at the start of every session). The **"now" marker stays pinned at the right edge** throughout ramp-up, consistent with the existing right-edge rule above. All windows remain **uniform-width slots within the fill** (confident and no-read alike), so a no-read gap's width stays **proportional to its consecutive-no-read count** — the honest elapsed-time proxy is preserved, just **scaled to the current fill** rather than to the absolute locked pitch. As each new window arrives during ramp-up, the existing points **gently re-space (compress)** to make room; this per-poll re-spacing is **by-design**, the accepted cost of avoiding blank-left. Once the count reaches **N_target**, the slot pitch **LOCKS** at `plot_width / (N_target − 1)`; from then on the oldest windows **scroll off the left edge** with no further compression (the steady-state behaviour above, unchanged). The ramp-up fill pitch is `plot_width / (count − 1)`, which **equals the locked pitch at exactly count = N_target**, so the pitch is **continuous at the lock point** — no visual jump at the transition. **Scope:** this clause governs **horizontal slot pitch only**; it does NOT change vertical band encoding (FR-003), the dashed warming-line treatment (FR-010), the no-read gap / no-bridging rules (FR-012/FR-013), or now-marker parking (FR-004a/FR-004b) — all unchanged.
...
- **FR-010**: A **leading run of no-read windows at session start** (no confident band yet, no skip cause) MUST render as a **dashed muted line** ("warming up") — a line, not a gap, and muted (not a stress signal). The dashed line requires **≥2 warming points** to render (a line needs two points); with **exactly 1 warming point** there is **no drawable line**, so that case shows the just-started text state instead (FR-018; *clarified 2026-06-27* — suppresses the lone right-edge stub that otherwise snaps to a full-width line on the next poll).
...
- **FR-018** *(1-warming-point carve-out 2026-06-27)*: A session with **zero trend points** (nothing captured yet) — **or exactly one warming point** (a single `band null & skipCause null` window, no confident reading yet, so **no drawable line**) — MUST keep the current **text-only** treatment ("Your trend builds as readings come in." — no axes, no line, **no stub**). The empty-text discriminator is therefore **"zero trend points OR exactly one warming point (no drawable line yet)"** — **not** "zero confident readings" (today's `drawableCount === 0`), which would wrongly match *any* warming-only/all-skipped session. A **warming-only session with ≥2 warming points** MUST render the dashed warming line (FR-010); an **all-skipped** session (≥1 *skip* point) MUST render its muted no-read state — never the empty text. The **subtitle** in the 1-warming-point text state continues to follow FR-024 (warming → "getting a read") — unchanged. (FR-010; Edge Cases "fully read-less"; SC-010.)
...
- **FR-024** *(clarified 2026-06-25; subtitle no-read rule decided 2026-06-25)*: The graph's **auto-derived subtitle** (the one-line summary above the trend — e.g. "A tense stretch in here." / "A little tension creeping in." / "Settled so far.") is **retained and in scope**. It MUST obey the same honesty rule as the rest of the graph: it MUST **never assert a tension level when there is no current confident reading**. By state:
  ...
  This mirrors feature 009's `deriveHeadline` honesty fork. *Subtitle wording signed off 2026-06-25 (Mohamed): neutral no-read line = **"No clear read right now"**; warming reuses FR-022's **"getting a read"**; confident-summary lines are the existing mock strings. No copy item remains pending.*
...
- **SC-010**: A single-reading session renders one dot (never a line); a fully read-less session renders a no-read state (never calm/at-ease). *(The load-bearing guarantee is **never calm/at-ease**. Per FR-018 the read-less **1-warming-point** case shows the just-started **text** state rather than a dashed line — still honest and never calm — so this case satisfies SC-010 via the text state, not a dashed/gap no-read treatment.)*
...
- **SC-012**: In a session longer than the rolling window (**steady-state regime, window count ≥ N_target**), the graph shows only the most recent ~2 minutes' worth of capture windows; each window occupies one uniform slot (no progressive compression as the session grows), a no-read gap's width equals its count of consecutive no-read windows, and the gap labels remain legible — if density forces a tradeoff, the oldest windows are dropped, not the slot width (FR-002a). *The "no progressive compression" guarantee scopes to this steady-state regime; the ramp-up phase (count < N_target) intentionally re-spaces per SC-012a.*
- **SC-012a** *(ramp-up fill, 2026-06-27)*: During **ramp-up** (window count < N_target = 12, ~the first 2 minutes at the ~10s window stride), the visible windows **fill the full plot width** — earliest at the left edge, latest at the right edge, with the "now" marker at the right edge — so there is **no blank-left "cut off" look**. The gentle per-poll re-spacing in this phase is **by-design** and is **distinct from** the steady-state "no compression as the session grows" guarantee (SC-012). The fill pitch is continuous with the locked pitch at the N_target hand-off (no visual jump). (FR-002a.)
