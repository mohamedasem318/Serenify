...
| CHK014 | popup dismissal on touch | Hover shows/hides on mouse-out; focus shows/hides on blur; **tap toggles** (second tap on the marker closes, an **outside tap dismisses**) — so pure `:focus-within` (the mock's mechanism) is **insufficient**: the popup needs an explicit **open/pinned state** (FR-007, reconciled 2026-06-27). While pinned the copy tracks the live marker state ("you are here" ↔ "last clear read"); no separate close control. (contract: now-marker) |
