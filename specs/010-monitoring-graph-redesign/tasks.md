...
- [X] T026 [P] Extend the existing monitor/employee Playwright happy-path in `apps/web/tests/e2e/` to assert a no-read treatment renders and **no numeric value of any kind** appears in the graph. [**SC-007**]
...
- [X] T028 Final verification: `cd apps/web && npm run lint && npx tsc --noEmit && npx vitest run --pool=threads tests/unit/lib/session-trend-geometry.test.ts tests/unit/components/monitor/session-trend.test.tsx` (Windows: `--pool=threads` per project memory); confirm the SC-001…SC-013 coverage map below is fully satisfied.
