...
- [ ] T001 [P] Verify `8dc822b` is in the branch's history: `git merge-base --is-ancestor 8dc822b HEAD` returns exit code 0. If not, rebase onto `main` before continuing — otherwise the references to `login-expired-link.spec.ts` and the CHANGELOG supersession note both become inaccurate. NOT a code task.
