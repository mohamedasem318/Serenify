...
- [ ] No red color appears anywhere in any rendered authentication state,
      including form errors (constitution Principle V).
- [ ] Both light and dark modes were tested for every authentication
      surface (signup, sign-in, sign-out, password reset, profile
      completion) — they are equal-priority (constitution Principle VI).
- [ ] `prefers-reduced-motion` was verified to suppress any motion on
      authentication surfaces (constitution Principle VI).
- [ ] No `.env*` files were committed; no API keys, connection strings,
      or private hostnames appear in any diff (constitution Principle IX).
- [ ] No real teammate names were used as demo or test accounts
