...
- [X] T024 [P] Clean-swap audit: remove any orphaned pre-`[3]` rendering from `apps/web/components/home/today-view.tsx`; update/replace `apps/web/tests/unit/components/home/todays-checkin-card.us4.test.tsx` to the new surfaces; confirm `apps/web/components/home/todays-checkin-card.tsx` wiring is unchanged (still loads recap+trend RLS-as-user and renders `<TodayView>`).
- [X] T025 [P] Extend employee Playwright e2e `apps/web/tests/e2e/employee-dashboard-shell.spec.ts` — assert the today card expands/collapses and the expanded view shows axis level-labels with **no** bottom legend (happy path; Constitution VII role e2e).
...
- [X] T027 [P] Append a 009 implementation entry to `docs/PROGRESS.md`.
- [X] T028 Run `apps/web` lint + typecheck + full Vitest + the extended Playwright e2e; walk `quickstart.md` at desktop AND 360px; fix any regression. Confirm SC-001…SC-010 all covered. *(2026-06-23: lint = 2 known pre-existing errors / 0 new; tsc green; Vitest 641/641; test:layout 5/5 incl. T031; e2e dashboard-shell 3/3 on chromium. No regressions. SC-001…SC-010 all mapped. The desktop+360px visual quickstart walk is carried by `smoke-tests.md` — Mohamed's hand-run gate.)*
...
- **Polish (T024–T028, T031)** → after all desired stories. T031 (first-paint width-flash fix) depends on US2 (T017, the expanded plot) + the a7be539 layout guard; touches only `today-trend-plot.tsx` + the `test:layout` spec, so it runs [P] alongside the other polish tasks.
...
- SC-002 shapes-not-bars @ desktop+360px → T005, T013, T016 (full-width row → real drawing area), T031 (no 1008-derived first-paint width flash @ 360px — fixed-px width honoured from the first painted frame)
