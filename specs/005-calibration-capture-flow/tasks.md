...
- [X] T027 [US6] Overwrite-on-success-only + exit routing in the orchestrator (reuse the existing client write path; **no new DB**) — across stop / processing-failure / "Not now" / "Maybe later" the prior baseline is untouched and the exit follows `mode`. (FR-053, DECISION-22)
