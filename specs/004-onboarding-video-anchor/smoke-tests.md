...
### ST-17 — Completing in tab A refreshes tab B
- **Steps**: Open the onboarding anchor step in two tabs (same session). Complete the recording in tab A.
- **Expected**: Tab B refreshes and lands on `/app` (off the now-completed step) within ~2s, without a manual reload.
- Result: ___  Date: ___  Notes: ___
