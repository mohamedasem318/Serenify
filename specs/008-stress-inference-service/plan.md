...
**Performance Goals**: First smoothed reading within ~90–105 s; a refreshed reading ~every 10 s thereafter (SC-001). Per-window CPU extraction ~3–5 s on the dev laptop (MediaPipe is CPU-bound), comfortably inside the 10 s stride. Under continuous upload the server also **decodes the growing clip to reach its trailing 60 s**, a cost that grows with elapsed session time (bounded by the 5-min cap; negligible on localhost; could exceed the 10 s stride late in a 5-min session on the droplet — flagged in research R-5, mitigation = the deferred rolling decoded-frame buffer). A slow window must never stall the next window's capture/upload (FR-016, SC-007).
...
- ML fixture regression: the webm/VFR **codec** fidelity check (CFR mp4 vs VFR webm) stays **scheduled hardening, not a ship blocker** (Test Plan Notes, R-6). There is **no multi-clip assembly fidelity gate** — the continuous 60 s window is faithful by construction (R-5); a small unit test pins the new tail-window option (it reduces to `compute_anchor` for a ≤ 60 s clip).
...
└── tests/
    ...
    ├── test_tail_window.py                 # NEW — pins the tail-window option (last-60 s bound; reduces to compute_anchor for a ≤60 s clip)
