...
- **US4: today recap + expand-in-place, retrospective (revised).** After a session ends, the dashboard check-in card recaps **today's** check-ins — a calm, band-derived templated headline plus a today **mini-trend** — and **expands in place** (no separate route or page) to the full today view: an **auto-fit, local-time** axis spanning the day's first-to-last reading, **numbered sessions** at their real times with honest **gaps** where unmonitored, a **peak marker** on the tensest stretch, and a per-session **timeline** (time-identity + state chip + short band-derived phrase). The monitoring page keeps the **live this-session** trend; the card's collapsed mini-trend and its expanded view read the **same persisted rows**, so they always agree (SC-008). Calm recedes and amber/tense catches the eye (Principle V); a session that produced **no readable band** reads honestly as "no clear read" (never calm), and an n=1 session renders as a single point. The card↔timeline **cross-highlight** is an additive, hover-and-focus, reduced-motion-safe enhancement. Today is **retrospective only** — a currently-live session is never drawn in it. (Reads/render rules in `data-model.md` § Reads; visual intent in `serenify-008-us4-inline-mock-v5.html`.)
- **One active session per user — enforced invariant (already implemented).** "At most one active (`ended_at IS NULL`) monitoring session per user" is a **DB invariant**, not just an assumption: a **partial unique index** `monitoring_sessions_one_active_per_user_idx (user_id) WHERE ended_at IS NULL` (migration `583049b`) plus a **last-tab-wins create route** that finalizes any prior active session as ended with `end_reason='abandoned'` (`ended_at` stamped at that session's last reading, else `now()`) **before** inserting the new run, recovering an index-race loss (`23505`) with one finalize+retry rather than a 500 (route `873c3c5`). The US4 today recap reader **tolerates a stale-active row** (treats `status='ended'` **OR** active-but-last-reading-> 5-min-old as retrospective) so a crashed/closed-tab orphan never shadows the recap, while a fresh-live session is excluded. (FR-032; `data-model.md` § Reads.)
...
**Scale/Scope**: Graduation-project scale (tens of demo users, **at most one active session per user — now an enforced DB invariant**, not just an assumption: partial unique index + last-tab-wins create route; see the Summary invariant bullet and `data-model.md`). One new backend router, one user-context (forwarded-JWT) data layer, two tables + one `SECURITY DEFINER` function, a thin tail-window option on the existing `compute_anchor` (no new extraction path), one new employee route + the check-in card states, reusing the calibration capture/detector stack.
...
│   │   └── session-trend.tsx               # UNCHANGED IN PURPOSE — the monitor-page this-session (live) trend
│   ├── home/todays-checkin-card.tsx        # collapsed recap (headline + today mini-trend + Start / "View today") AND the in-place expanded today view (auto-fit local-time plot, numbered sessions, peak marker, per-session timeline, cross-highlight, read-less, n=1 dot, empty state, calibrate-first). INLINE — NO new route/page. (a home/today-view.tsx sub-component is acceptable if cleaner)
...
│   ├── api/monitoring-reads.ts             # NEW — typed Supabase RLS reads. keep getSessionTrend(sessionId) (monitor-page this-session); ADD getTodayTrend(userId) + getTodayRecap(userId): today's RETROSPECTIVE sessions = ended OR stale-active (no reading in >=5 min), EXCLUDING the fresh-live session; owner columns only (band, captured_at, scored, skip_cause, session_id — never label/stress_probability); per session the derived time-identity, state chip, short phrase, read-less flag (no readable band); plus the day span for auto-fit. (data-model.md § Reads)
...
└── 20260621000000_one_active_session_per_user.sql        # SHIPPED 2026-06-21 (commit 583049b) — one-active-session invariant: partial unique index (user_id) WHERE ended_at IS NULL + backfill + terminal end_reason 'abandoned' (FR-032)
```
...
- **Growing-decode keep-up — SURGICAL O(stride) tail-decode SHIPPED (2026-06-21); the full rolling buffer is now only a conditional upgrade.** The original read path re-decoded the **whole recording-so-far** each stride to reach its trailing 60 s — a cost that grows with elapsed session time (R-5; the supervised smoke measured the lag *growing* ~9 s/window to ~3 min behind). Rather than the full rolling-frame buffer, this was fixed **surgically** in `ml-video`: both the `< 60 s` gate (`probe_recorded_seconds`) and the tail decode (`compute_anchor(tail_seconds=60)` → `pipeline._extract_landmarks_tail`) now touch only the **bounded trailing 60 s** — a cheap ffprobe packet read gives the file-global sampling grid + duration (no whole-file grab), and only the trailing window is decoded (OpenCV native seek for mp4; an `ffmpeg -c copy` lossless tail remux for un-finalized webm, whose `cap.set` seek is a silent no-op). Proven **bit-identical** to the whole-file path on the real continuous fixtures (`tests/test_tail_seek_keepup.py` — the permanent guard; CI suffix-invariant stays T006). Per-window cost went from **O(elapsed)** (18→55 s across 62→301 s, lag climbs) to **O(stride/window)** (flat ~9–13 s). This addresses **only the growing-decode component**; the **constant per-window extract** (MediaPipe + LBP, ~2–3 s) is the separate fixed cost the fix does **not** touch — an extract-bound breach still calls for a slower cadence or GPU MediaPipe (R-5). Adds an **ffmpeg/ffprobe CLI** host dependency (Dockerfile + `apps/api/README.md`); absent → degrades to the old whole-file decode, runs-but-fails → skipped window. The **full per-session rolling decoded-frame buffer** (decode only the **newest ~10 s increment** each stride → true O(stride), not O(window) ~9–13 s) remains the upgrade to build **only if** keep-up re-measured on the chosen deploy target breaches the stride there. Pairs with the future WebSocket transport. See `docs/BACKLOG.md` (008 keep-up entry) + `docs/DECISIONS.md` (2026-06-21).
