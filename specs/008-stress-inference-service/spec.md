...
- **D-4 — Readings persistence schema** — **RESOLVED**: two tables (`monitoring_sessions`, `window_readings`) with RLS select/insert/update-own and **no manager policy**, the raw `stress_probability`/`label` held **server-only** via the SELECT column whitelist, **90-day** retention for readings, and the shared `getSessionTrend` / `getTodayTrend` / `getTodayRecap` reads. See [`data-model.md`](./data-model.md).
