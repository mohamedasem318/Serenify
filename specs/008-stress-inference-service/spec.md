...
**Independent Test**: With a calibrated employee account, start a check-in, grant the camera, and confirm a smoothed three-state reading appears on the bloom within ~60–75 seconds and refreshes roughly every 10 seconds — without any numeric value being shown anywhere.
...
2. **Given** an active session with the user in frame, **When** the first 60-second window completes, **Then** a smoothed reading is shown as exactly one of **At ease**, **A little tense**, or **Tense**, with no numeric percentage or gauge anywhere on screen.
...
- **Thin / low-coverage window**: a window without enough usable face (per the feature-006 usable-face-coverage gate) returns a clear "couldn't read this window" outcome through the existing extraction-failure channel. The UI treats it as out-of-frame / skipped and shows no reading — never a fabricated one.
...
- **First-window latency**: nothing is shown as a "reading" before the first full 60-second window completes; the UI communicates that the first read is still forming rather than implying a neutral result.
...
- **FR-004**: The UI MUST represent these operational states distinctly: **permission**, **active** (recording, in frame), **out-of-frame** (auto-paused, self-view shown, foggy framing prompt), **paused** (manual break, camera released), **blocked**, and **ended**.
...
- **FR-013**: A window without enough usable face (feature-006 usable-face-coverage gate) MUST return a clear "couldn't read this window" outcome via the existing extraction-failure channel; the UI MUST treat it as out-of-frame / skipped and MUST NOT fabricate a reading.
- **FR-014**: The displayed state MUST be smoothed across recent windows so it drifts rather than flickers. The spec defines the smoothing **policy** (a rolling smoothing over recent windows mapped to three bands — At ease / A little tense / Tense); the exact recent-window count and band thresholds are deferred to planning (see Deferred Decisions).
...
- **FR-019**: The idle dashboard check-in card MUST show a recap of the user's last session.
...
- **SC-001**: A calibrated employee starting a check-in sees their first smoothed reading within roughly 60–75 seconds (one full window plus extraction), and a refreshed reading approximately every 10 seconds thereafter.
...
- **SC-010**: All six operational states (permission, active, out-of-frame, paused, blocked, ended) are reachable and visually distinct, using the Principle V color roles (amber = stress only; foggy = attention/error; meadow = calm/affirmative).
...
These are flagged for planning per the brief and are intentionally **not** resolved in the spec. They are architecture/implementation choices, not scope ambiguities.

- **D-1 — Anchor read path at inference**: server-side service-role read keyed by the authenticated `user_id` (maintainer's current lean, since only the server reads the anchor) **vs.** a self-scoped `SECURITY DEFINER` function. Both should be evaluated; the former is recommended.
- **D-2 — Endpoint shape**: a single stateless inference endpoint **vs.** a session-aware design (session create/end plus per-window scoring). The trade-off (statelessness/simplicity vs. server-side session grouping and lifecycle) should be weighed in the plan.
- **D-3 — Smoothing & banding specifics**: the exact recent-window count and the thresholds mapping the smoothed signal to the three states (At ease / A little tense / Tense). The policy shape is fixed in FR-014/FR-015; the numbers are for the plan.
- **D-4 — Readings persistence schema**: the concrete session + readings tables, retention policy, and how the trend queries aggregate them for the card and the page.

### Open Questions — Mock Gaps (mocks-first governance)

- **MG-1 — Calibrate-first surface**: the approved mock shows permission and blocked panels but **no** "you need to calibrate first" surface for the no-anchor outcome (FR-011 / User Story 3). The exact visual, copy, and call-to-action for this state are not specified by the mock. This is recorded as an open question for the mock owner rather than invented here.
- **MG-2 — "Couldn't read this window" treatment**: the brief routes a thin/low-coverage window to the out-of-frame / skipped treatment, but the mock does not show a distinct visual for a coverage failure that occurs while the user appears to be in frame. Confirm whether it reuses the out-of-frame surface verbatim or needs its own quiet "skipped a read" affordance.
- **MG-3 — Monitoring stage at mobile width**: the mock's grid collapses at ~760px, but the monitoring stage (bloom, pill, viewfinder, trend) is presented at desktop width. The ≥360px layout (Principle VI) for the monitoring stage is not pinned down by the mock; confirm the mobile arrangement of pill/viewfinder/controls.

## Test Plan Notes

- `predict_delta` currently has **zero tests**; this feature adds its first caller and its first test.
- **webm/VFR fidelity residual (planned hardening check, not a blocker)**: decode the same content as both mp4 (CFR) and webm (VFR), then confirm the extracted 2958-d vectors stay within tolerance. Real inference clips are Chrome MediaRecorder webm/VFR, which travel a timestamp-sampler path the notebook fidelity gate did not check. This is a hardening item to schedule, not a gate on shipping the feature.
...
- **Principle VII (Mandatory Testing)**: first `predict_delta` caller and test plus the webm/VFR hardening check (Test Plan Notes).
