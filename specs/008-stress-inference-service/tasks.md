...
- [x] T042 [US3] `apps/api/app/routers/monitoring.py`: finalize the **no_anchor** outcome — never substitute a global/fallback anchor (FR-011, SC-004). Split into its two halves, **both done**:
  - [x] **T042a — create-time 409** `{"outcome":"no_anchor"}` when `get_my_anchor()` is NULL at session create (done in **T021**; the no-anchor employee can never start a session and is routed to calibrate-first up front).
  - [x] **T042b — defensive mid-session 409** (done in this task): `POST …/windows` catches `MissingAnchorError` from `score_window` and returns the **same** `{"outcome":"no_anchor"}` body — never a 500, never a reading without the user's own anchor, if the anchor disappears mid-session.
- [x] T043 [P] [US3] `apps/api/tests/test_monitoring_endpoints.py` (US3 slice) **+ web**: the **mid-session** gap (the create-time 409 / SC-004 create-side is already covered by **T024**). Backend: a scored window whose anchor has vanished → **409 no_anchor** and **persists no `window_readings` row** (never a reading without the user's own anchor, SC-004). Web: `submitWindow` disambiguates the 409 body (`no_anchor` vs `ended_session`), and the orchestrator routes a mid-session `no_anchor` → the existing **calibrate-first** state.
- [x] T044 [US3] `apps/web/components/monitor/op-surfaces.tsx` + `use-monitoring-session.ts`: **calibrate-first** foggy panel (attention, not stress) with a short line and a **meadow "Start calibration"** forward action routing to the calibration flow; the `no_anchor` outcome → calibrate-first state, **never** a band. (mock-gap #4 / MG-1; FR-011, Principle V) — **Done (built in US1's create-path slice — `CalibrateFirstPanel` + the reducer `NO_ANCHOR` → calibrate-first; reused unchanged for the T042b mid-session route).**
- [x] T045 [P] [US3] `apps/web/tests/unit/components/monitor/` (US3): `no_anchor` → calibrate-first surface rendered, no stress band rendered. — **Done (reducer `NO_ANCHOR` → calibrate-first + op-surfaces calibrate-first panel covered in US1; this task added the explicit "no stress band / no bloom rendered" assertion).**
...
**Goal**: A **today** recap on the dashboard check-in card that **expands in place** (no route change) to the full day trend + per-session breakdown — its collapsed mini-trend and expanded view from the **same** persisted rows (SC-008) — with read-less honesty, an empty/calibrate-first state, and an auto-fit local-time axis; the monitoring page keeps the **live this-session** trend; the persisted shape sufficient for feature 009's sustained-tense detection (seam only — no questionnaire here).

**Independent Test**: Complete one or more sessions in a day, then confirm the check-in card recaps **today** and **expands in place** to a day trend whose mini-trend and expanded view come from the **same** persisted rows; a read-less session shows as "no clear read" (never calm); an n=1 session renders as a single point; the day axis auto-fits in local time; a live session is **not** drawn in today; and `window_readings` carries `band`+`captured_at`+`session_id` for 009.

- [x] T046 [P] [US4] `apps/web/lib/api/monitoring-reads.ts` (NEW): typed Supabase **RLS** reads. **Keep** `getSessionTrend(sessionId)` (monitor-page this-session; `select id, captured_at, scored, band, skip_cause`; never the probability). **Add** `getTodayTrend(userId)` + `getTodayRecap(userId)` per B2 — today's **retrospective** sessions = **ended OR stale-active** (last reading more than 5 minutes ago), **excluding** the fresh-live session; owner columns only (`band, captured_at, scored, skip_cause, session_id`; never `label`/`stress_probability`); per session the derived **time-identity / state chip / short phrase / read-less flag** (no readable band); plus the **day span** for auto-fit. (data-model.md § Reads; FR-018/019 + the new local-time FR-030 + retrospective FR-031 + one-active-session FR-032 — recap tolerates a stale-active row) — **Done.** Three readers over the browser RLS client (no service-role); `getSessionTrend` = `select id, captured_at, scored, band, skip_cause`, `getTodayTrend`/`getTodayRecap` = owner columns only (`band, captured_at, scored, skip_cause, session_id` / `id, started_at, ended_at, status`), never `label`/`stress_probability`. Pure derivations (`isReadLess`, `bandCount`, `sessionTenor`, `retrospectiveSessionIds`, `deriveRecap`, templated headline) are exported + unit-tested. Retrospective = ended OR stale-active (last reading >5 min); fresh-live excluded.
- [x] T047 [US4] `apps/web/components/monitor/session-trend.tsx` (NEW): the **monitor-page this-session** live trend (band→height; **skipped = gap, never fabricated or carried-forward**; n=1 = single point); uses the shared `getSessionTrend` reader. (FR-018 + the new no-fabrication FR-029) — **Done.** Polls the shared reader while live (injectable `load`/`active`); runs of consecutive bands render as line segments, a skip breaks the line (gap), a lone band renders as a dot. **Mounted on the monitor page** via a minimal additive change to the US1/US2 orchestrator `monitoring-session.tsx` (a `liveSessionId` state + a stable injected loader so the orchestrator unit tests stay network-free) — the one US1–US3 file touched, additive only.
- [x] T048 [US4] `apps/web/components/home/todays-checkin-card.tsx`: collapsed recap — a **templated headline** from today's bands + times (band-only; never the probability) + today **mini-trend** from `getTodayTrend` + **Start check-in** + **"View today"**; and the **in-place expanded today view** — auto-fit **local-time** axis, **numbered sessions** with honest gaps, a **peak marker** on the tensest stretch, a per-session **timeline** (time-identity title + state chip + short templated phrase; **read-less = "no clear read"**; **n=1 = single dot**), the **cross-highlight** linking a session's plot badge and its timeline row on **hover and keyboard focus** (additive; reduced-motion-safe via the repo `useMediaQuery`), the **empty state**, and **calibrate-first** when the user has no anchor. Calm recedes, amber/tense catches the eye (Principle V). Expand/collapse animates height with a reduced-motion fallback; the toggle is keyboard-operable with `aria-expanded`. (a `home/today-view.tsx` sub-component is acceptable if cleaner.) (FR-018/019 + FR-028…FR-031, mock-gap #7) — **Done.** Built to the v5 mock in real Graphite tokens: a pure `today-view.tsx` (templated headline with the tense clause in amber; auto-fit local-time axis; numbered session badges as HTML overlays; peak marker; per-session timeline ordered tense-first with read-less = "no clear read"; n=1 = single dot; cross-highlight on hover **and** keyboard focus; height-animated expand with a `useMediaQuery` reduced-motion fallback; `aria-expanded` toggle) + the `todays-checkin-card.tsx` client wrapper that branches on `has_anchor`/today's sessions (calibrate-first / empty / recap) and is fed `userId`+`hasAnchor` from the dashboard page. The collapsed mini-trend and the expanded plot read the same `getTodayTrend` rows (SC-008). The card's prior Start-check-in / calibrate-first default behaviour is preserved.
- [x] T049 [P] [US4] `apps/web/tests/unit/components/` (US4): collapsed mini-trend ↔ expanded today view **consistency** (same source); **read-less never renders as calm** (SC-011); **n=1 = single dot**; empty-state + **calibrate-first** branch (`has_anchor`); templated headline/phrase derivation is **band+time only**; **retrospective-only** (fresh-live session excluded). — **Done.** TDD red→green: `tests/unit/lib/monitoring-reads.test.ts` (derivations), `tests/unit/components/monitor/session-trend.test.tsx`, `tests/unit/components/home/today-view.test.tsx`, `tests/unit/components/home/todays-checkin-card.us4.test.tsx` — all listed assertions covered (40 new cases). Existing `todays-checkin-card.test.tsx` still green (default preserved). Full web suite **572 passed**; tsc + lint clean on all new/changed files.
- [x] T050 [US4] FR-020 **seam** confirmation (unchanged): assert the persisted `window_readings` shape (`band` + `captured_at` + `session_id`) supports the 009 sustained-tense query; `data-model.md` documents 008 builds **no** questionnaire trigger/UI/flow. (FR-020, Out-of-Scope) — **Done.** `tests/unit/lib/monitoring-reads-seam.test.ts` asserts the migration persists `band`/`captured_at`/`session_id` + the `(session_id, captured_at)` and `(user_id, captured_at)` indexes, the migration introduces no questionnaire object, the data-model documents the seam-only scope, and the reader column lists + the DB SELECT grant both withhold `label`/`stress_probability`. **No** questionnaire trigger/UI/flow built.
...
- [x] T051 [P] Playwright **employee happy-path** e2e `apps/web/tests/e2e/employee-monitoring.spec.ts` (start → permission → warming-up → reading → end → **dashboard today recap, expanded in place** — NOT a separate "today" page) using the feature-005 **detector-injection seam**. NOTE: this is **not** the cross-browser capture gate — that is the real-Safari smoke (Phase 2), which Playwright must not replace.
- [x] T052 [P] webm/VFR **codec** fidelity hardening `packages/ml-video/tests/test_webm_vfr_fidelity.py` (mp4/CFR vs webm/VFR 2958-d tolerance) — **scheduled hardening, not a ship blocker** (R-6; there is **no assembly dimension** under continuous single-stream — faithful by construction).
- [x] T053 [P] Responsive & a11y pass (Principle VI / FR-025): monitoring stage **stacks at ≥360 px** (bloom shrinks, controls full-width/stack, pill/viewfinder reposition — mock-gap #3); `prefers-reduced-motion` across all surfaces; visible keyboard focus; ≥44×44 px touch targets. **Also the US4 inline-expand toggle (keyboard-operable, `aria-expanded`) and the card↔timeline cross-highlight (hover + keyboard-focus parity), with `prefers-reduced-motion` honored on both the expand animation and the highlight transition.** Covered in `apps/web/components/monitor/*` + `apps/web/components/home/*` + the page.
- [x] T054 [P] Author/expand `specs/008-stress-inference-service/smoke-tests.md`: human checks — camera permission on real browsers; **Safari/iOS** secure-context + capture; HTTPS/localhost; low-light skip; mobile 360 px; reduced-motion; privacy (temp/clip deleted); no manager surface reads sessions/readings.
- [x] T055 Privacy verification (Principle I / Quality Gate 6): test + smoke that **no raw video persists** (the uploaded clip + temp deleted in `finally`; no clip buffer), **no manager policy** exists on either table, and `label`/`stress_probability` are **unreadable** by the owner (SELECT column whitelist). (SC-009)
- [x] T056 [P] **Model-owner note** (carry-over, do **not** act in 008): record the `metadata.json` stale `window_eval_config` (30 s) cleanup as a model-owner task — **metadata/doc-only, no `model_version` bump, no anchor invalidation, do not edit the model artifact**. Add to `docs/backlog.md` (or the MODELS.md note) flagged for the model owner. (research R-0)
- [x] T057 [P] **Retention follow-up note**: document the 90-day `window_readings` purge (a `pg_cron` job or scheduled task) as a follow-up **not built in 008**; the policy is decided, the job is deferred. (data-model.md § Retention)
- [x] T058 Run `quickstart.md` verification (SC-001…SC-011) and the full Principle VII test sweep (pytest `apps/api` + `packages/ml-video`; Vitest `apps/web`; Playwright employee e2e; the tail-window unit test + webm/VFR hardening). Confirm green before review.

---

## Shipped ahead of US4 (completed)

> Recorded for traceability (FR-003): this invariant was implemented during the 2026-06-21 edge-case pass, ahead of the US4 build, because the today recap depends on it. It is **done**, not pending.

- [x] T059 **One active session per user — enforced DB invariant.** Migration `583049b` (`supabase/migrations/20260621000000_one_active_session_per_user.sql`) adds the **partial unique index** `monitoring_sessions_one_active_per_user_idx (user_id) WHERE ended_at IS NULL` (+ a backfill finalizing pre-existing duplicate actives so the index builds; new terminal `end_reason='abandoned'`, CHECK extended). The **create route** `873c3c5` (`apps/api/app/routers/monitoring.py` + `supabase_user.py` helpers) finalizes any prior active session as **'abandoned'** (last-tab-wins; `ended_at` stamped at that session's last reading, else `now()`) **before** inserting the new run, recovering an index-race loss (`23505`) with one finalize+retry — never a 500. The **US4 today recap reader (T046) tolerates a stale-active row** (`status='ended'` **OR** active-but-last-reading->5-min-old ⇒ retrospective; the fresh-live session excluded), so a crashed/closed-tab orphan never shadows the recap. No service-role; RLS + the SELECT column whitelist + append-only readings unchanged. (FR-032; `data-model.md` § Reads; DECISIONS 2026-06-21.)
...
- T046 (reads: `getSessionTrend` + `getTodayTrend`/`getTodayRecap`) → T047 (monitor-page this-session trend) + T048 (today recap + expand-in-place).
