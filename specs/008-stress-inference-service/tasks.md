...
- [ ] T046 [P] [US4] `apps/web/lib/api/monitoring-reads.ts` (NEW): typed Supabase **RLS** reads. **Keep** `getSessionTrend(sessionId)` (monitor-page this-session; `select id, captured_at, scored, band, skip_cause`; never the probability). **Add** `getTodayTrend(userId)` + `getTodayRecap(userId)` per B2 — today's **retrospective** sessions = **ended OR stale-active** (no reading in ≥5 min), **excluding** the fresh-live session; owner columns only (`band, captured_at, scored, skip_cause, session_id`; never `label`/`stress_probability`); per session the derived **time-identity / state chip / short phrase / read-less flag** (no readable band); plus the **day span** for auto-fit. (data-model.md § Reads; FR-018/019 + the new local-time FR-030 + retrospective FR-031)
