...
2. Keep all three on localhost: `NEXT_PUBLIC_API_URL` (apps/web `.env.local`,
   default `http://127.0.0.1:8000`), the API's `ALLOWED_ORIGIN` (apps/api `.env`,
   `http://localhost:3000` — it must equal the origin you load), and the origin you
   open in the browser.
...
`NEXT_PUBLIC_API_URL` and a matching `ALLOWED_ORIGIN` (e.g. `http://<lan-ip>:3000`),
with the API bound to `0.0.0.0`. Stand up HTTPS (a dev tunnel or a deploy preview)
