...
(Python 3.12, ``mediapipe==0.10.13``), to turn the three real calibration clips into
committed landmark arrays:

    uv run python tests/fixtures/extract_coverage_fixtures.py \
        --thin <thin.mp4> \
        --good-ideal <good-ideal.mp4> \
        --good-realistic <good-realistic.mp4>

It writes ``{thin,good_ideal,good_realistic}.npy`` next to this file and prints the
usable / kept / fraction table (T013). The raw clips are NEVER committed
...
CLIPS = ("thin", "good_ideal", "good_realistic")
