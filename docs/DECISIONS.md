...
**Open reconciliation flagged for the 013 spec — BACKLOG `#75` ↔ `013` ownership**: the ToS/Privacy/consent-gate item `#75` was mechanically renumbered `015 → 018` (keeping its `privacy-controls-and-transparency` slug per the sweep instruction), but its actual subject — the ToS/Privacy documents and the signup consent gate — is **exactly the scope of the new `013-public-surface-and-legal`**. #75's "draft alongside privacy-controls" rationale (privacy-controls defines the data-handling substance the documents describe) is real, but ownership of the *documents themselves* now overlaps 013. This was **not** re-slotted here (a re-scope, beyond "fix the number"); it should be reconciled when `013-public-surface-and-legal` is specified — decide whether #75 becomes a 013 deliverable, an 018 deliverable, or is split across the two.
