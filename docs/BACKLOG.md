...
**Address by**: feature 018 (admin-dashboard) or whichever feature first ships to a
...
users (likely feature 011 / chatbot interrupts, or feature 015 /
...
(feature 018) this is table-stakes provenance.
**Fix scope**: small-to-medium. Two viable shapes:
  ...
  - (b) A dedicated `public.invite_audit` table (`id`, `invited_by`,
    ...
    succeeds.
**Address by**: feature 018 (admin-dashboard), which is the first consumer that
...
**Address by**: when `/api/admin/invite` gets a real browser client (feature 018
...
the thesis/demo stage. Likely pairs with feature 018 (admin-dashboard), which
owns the invite UX.
...
feature 018** — calibrating a limit without a real admin UI is arbitrary, and
...
with feature 018 (admin-dashboard), when `/api/admin/invite` gets a real browser
...
**Address by**: draft **alongside feature 015 (privacy-controls-and-transparency)**, since
