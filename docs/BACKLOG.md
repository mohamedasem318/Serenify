...
- **iOS Safari *monitoring* produces 0 readings — a server-side decode death on the iOS un-finalized webm (`probe_recorded_seconds` throws → `our-side`); NOT the resolved kickout** (#89) (`bug` / suggested labels `type:bug` + `area:ml-video` (decode/probe lives there; caller in `apps/api`) + **not demo-blocking**; from 008 device gate T009 + smoke **Run 4, 2026-06-22**): On a **real iPhone Safari over HTTPS** (Run 4, cloudflared tunnel) app load + sign-in + on-device **calibration all worked**, but a **monitoring** session produced **0 readings** and died ~3.5 min in showing the client surface **"This one was on our side."** **Symptom:** the session fails partway in with the "our side" server error and never scores. **Root cause (precise):** `probe_recorded_seconds()` — the ffprobe duration probe in the O(stride) tail-decode path (**defined in `packages/ml-video/src/ml_video/anchor.py`**, called from `apps/api/app/services/inference.py`) — **throws on iOS Safari's un-finalized *growing* webm past the early-seconds mark (~40 s)**, tripping the scoring gate. Server trace (session `02a60299`): windows `probe_s = 9 → 19 → 29 → 39 s` `warming_up`, then **`probe_s = -1.00 … FeatureExtractionError:our-side`** repeated → every window skipped → the session **never reaches the 60 s scoring gate** → 0 readings. iOS also records **sub-realtime** (`probe_s = 39` at ~75 s wall-clock). **NOT auth / NOT the kickout (which is resolved):** every window upload returned **200**, **0 × 401, no re-login** — auth held the whole ~3.5 min (so the 008-followups `[1]` stale-token fix held on iOS and was the wrong layer for this); a true ITP/re-login kickout did **not** reproduce. Chrome's un-finalized webm decodes fine (desktop scored **35 readings** across the full band range the same day) — **iOS's webm is the problem child**, the same container **T009** flagged. **Impact — disproves the old assumption:** this is **NOT** the old "iOS live-readings are blocked until a real HTTPS deploy" read — Run 4 *had* HTTPS and load/sign-in/calibration all passed; it is a **capture/decode** failure, not a transport/HTTPS one. **Likely fix direction — same problem class as the known un-finalized-`MediaRecorder`-webm issue** (there the fix was an **`ffmpeg -c copy` remux** to a finalized/cued container *before* OpenCV decode, because `cap.set()` is a silent no-op on un-cued webm): **(1) prefer fMP4/CMAF on the iOS *monitor* recorder** — **T009 proved this iPhone's fMP4/CMAF decodes cleanly to `(2958,)` where its webm chokes** (the recorder's `pickMime` already needs webm-first-with-fMP4-fallback per the T026 backlog item above) — and/or **(2) harden `probe_recorded_seconds`** to degrade gracefully on an un-finalized iOS webm (remux-then-probe, or fall back to a timestamp estimate) instead of throwing `our-side`. Then **re-run ST-08-2**. Full evidence: `specs/008-stress-inference-service/smoke-tests.md` **Run 4** (commit `34c951b`). Fix scope: small-to-medium (recorder-mime preference + a probe-hardening guard + a T009 fMP4 re-validation on the deploy image). **Non-blocking for the desktop demo; required for real iOS monitoring support.** Address by: before any real-iOS monitoring sign-off; pairs with the **T026 recorder-mime** + the **keep-up tail-decode** items above (same decode path). **[Fix LANDED 2026-08-05 (Phase 2, PR #243): both recorders now negotiate fMP4-first on Apple WebKit via the shared `apps/web/lib/capture/constraints.ts` (Safari 26 claims webm — the exact routing this entry predicted — while its mp4 recorder chunks on schedule with a realtime clock), and the REAL iPhone growing-fMP4 fixture passes probe + tail decode + full feature build (`packages/ml-video/tests/test_growing_fmp4_decode.py`). Mechanism refinement: the June growing-iOS-webm fixture probes fine AT REST — the live death implicates iOS webm's transport scale as much as the container. **Wire-weight correction (2026-08-07):** the "fMP4 is ~5× lighter" figure was derived at 480×640 and does not scale — re-measured at the shipped 720p operating point, iOS Safari fMP4 is ~40 MB/60 s (~5.4 Mbit/s), essentially the same as the WebM probe's ~45 MB. The container swap is justified by decode correctness, not by wire weight. See `docs/DECISIONS.md` 2026-08-07.]** **RESOLVED 2026-08-08 — the ST-08-2 re-run passed on a real iPhone (iOS 18.7 / Safari 26.5.2) against production `a0ba6fd` (PR #250, the Apple-only 750 kbps `videoBitsPerSecond` target). GitHub issue #89 CLOSED in the same change (Principle VIII).** Evidence: a 6-minute session produced **15/15 windows scored, 0 skipped, 0 `our-side`** (22/22 across both of the day's sessions), every decode probe `outcome=ok branch=remux container=.mp4 … kept=150`, and **time to first reading 1:25 / 1:31** — at the ~90 s theoretical floor, beating the pre-fix laptop-Chrome control (1:37). The **2026-08-07 pre-fix control on the same account produced 0 readings in 5m17s**, managing only 2 window POSTs (~100 s apart, upload-bound, never reaching the 60 s gate); today's POSTs ran ~11–19 s apart. Causal attribution is unusually clean: both runs were served by the **same API revision** (`serenify-api--preview-cors-243`, 100 % traffic throughout) — the only variable that changed is the client bundle. So the decode-death mechanism this entry describes is gone: `probe_recorded_seconds` no longer throws, because the fMP4 container plus the 750 kbps target keeps each stride's upload light enough to carry real media. **Two caveats recorded, neither affecting this resolution:** (1) the validating session ran against a **21-hour-old anchor captured on `3410cb4` (WebM/VP9, ~6.7 Mbit/s)** while its windows were fMP4/H.264 at 750 kbps — a cross-container `window − anchor` mismatch that makes those *probability values* untrustworthy, though not the *production* of readings that #89 is about; filed separately. (2) Steady-state readings arrived **~19 s apart against the 10 s design stride** (server scoring ~4.5 s/window appears to serialize the client loop) — **stable, not growing**, so unlike #78/#110 this is a throughput observation, not a regression.
...
- ~~**Copy: collapse "a little tense" to a single word**~~ — **RESOLVED 2026-08-13 (superseded by the band rename, PR #262; issue #92 closed in the same change)** (#92) (`polish` / `area:web`, from 009 follow-up §3): the two-word mid-tense descriptor was the only multi-word band label; a single word would tighten the headline and simplify the partial-easing clause ("eased a little"). **Resolution:** superseded rather than fixed on its own terms — the full display rename to **Calm / Uneasy / Tense** landed "uneasy" as the single word everywhere (headline "Your afternoon turned uneasy", chips, axes, statelines), with the prose sites rewritten to read naturally. Enum keys unchanged. See `docs/DECISIONS.md` 2026-08-13.
...
## Monitoring — surfaced during the 2026-08-08 iOS post-fix verification session
...
**Step 1 landed — 2026-09-13 (PR #276)**: question **(b)** answered **yes** and done —
...
### Double-press on a mild confirmatory prompt silently swallows the later tense prompt
**Status**: bug (`type:bug` / `area:web`) — **OPEN.** GitHub issue **#271 OPEN.**
**Category**: confirmatory detection (feature 012)
**Observed**: 2026-08-18, by the T016 adversarial review of 014's dependency swap.
A double-press on a **mild** confirmatory prompt latches `resolvedRef`: press 1 re-arms
and clears it, press 2 sets it `true` with no prompt row to match. The session's later
**tense** prompt is then created but resolved `expired` / `session_end` without ever being
shown. **Pre-existing since PR #135** and reachable through the old `openRen` path too, so
it is not 014's defect — 014 only inherited it. The fix belongs in `finalize`'s re-arm,
which must not leave a latch that outlives its prompt.
**Fix scope**: small — one re-arm path, guarded by the existing resolution suite.
**Address by**: next time the confirmatory trigger is opened for any reason; before any
release that leans on the tense prompt.

### Windows `--pool=threads`: concurrent repo-scanning suites fail at collect
**Status**: tech-debt (`type:tooling` / `area:web`) — **OPEN.** GitHub issue **#272 OPEN.**
**Category**: test harness (Windows-only; environment, not product)
**Observed**: 2026-08-18, across 014's verification runs.
The repo-wide `readFileSync` scanner suites — `wordmark-sync`, `focus-indicator-guard`,
and the already-documented `hosted-email-template-sync` — intermittently fail on Windows
at the **collect** phase with a runner-level `STACK_TRACE_ERROR` and **no assertion diff**,
when several tree-walkers run concurrently. Each passes in isolation on a clean tree. CI
(ubuntu) is unaffected and no product code is implicated. Working rule: treat a failure in
this family as environment until an isolate-rerun says otherwise.
**Fix scope**: harness-only — likely a concurrency limit or one shared tree-walk fixture,
rather than three independent full-repo scans.
**Address by**: next time the test harness is opened; sooner if a Windows contributor is
blocked.
