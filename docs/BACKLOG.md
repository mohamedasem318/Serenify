...
- **Keep-up — the read loop needs back-pressure/cadence control IN ADDITION to the deferred rolling decoded-frame buffer** (`watch` / production-deploy, from 008 device gate; pairs with research R-5 / T009): the live ~3× contention (9.7 s standalone → 30 s with overlapping strides) previews production stride pileup once server decode > the 10 s stride, so the loop must not fire a new stride while one is in flight (or must coalesce) — the rolling buffer cuts per-stride decode but does not prevent overlap.
